...
const Relations: React.FC<RelationsProps> = ({ relations }) => {
    ...
    return (
        <div className="mt-10 rounded-2xl bg-surface overflow-hidden">
            <div className="flex items-center justify-between p-6 pb-4">
                ...
                {relations.length > 3 && (
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-[11px] font-bold text-white/50 transition-all hover:text-primary"
                    >
            ...
            <div className="flex flex-col gap-2 p-2">
                {displayRelations.map((rel, idx) => (
                    <Link
                        ...
                        className="group relative flex h-[80px] bg-card hover:bg-surface-raised rounded-xl overflow-hidden transition-all duration-300 active:scale-[0.98]"
                    >
                        {/* Left Content */}
                        ...
                        <div className="absolute top-0 right-0 w-[45%] h-full">
                            <div className="absolute inset-0 z-1 bg-gradient-to-r from-card via-card/60 to-transparent" />
                            <img
                                src={rel.entry.image}
